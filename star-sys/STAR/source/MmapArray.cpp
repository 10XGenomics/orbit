...
int MmapArray::initMmap(const std::string &filename, size_t length, size_t suffix_padding) {    
    ...
    file_mmap_length = round_up_to_page(length);
    size_t padded_file_mmap_length = round_up_to_page(length +suffix_padding);
    mmap_length = padded_file_mmap_length + PAGE;
    ...
    if (addr2 == (void*) -1 || (char*)addr2 != (char*)addr1 + PAGE) {
        ...
        munmap(addr1, mmap_length);
        close(fd);
        return -1;
    }

    // make the final page of the file writeable, to permit customization of
    // flanking bytes.
    // On Mac at least, it's much faster if the 'main' mmap above is read only.
    void* addr3 = mmap((void*) ((char*) addr2 + file_mmap_length - PAGE), 
        PAGE, 
        PROT_READ|PROT_WRITE, 
        MAP_FIXED|MAP_PRIVATE,
        fd, 
        file_mmap_length - PAGE);
    ...
    if (padded_file_mmap_length > file_mmap_length) {
         void* addr4 = mmap((void*) ((char*) addr2 + file_mmap_length), 
            padded_file_mmap_length - file_mmap_length, 
            PROT_READ|PROT_WRITE, 
