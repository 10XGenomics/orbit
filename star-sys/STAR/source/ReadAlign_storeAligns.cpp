...
void ReadAlign::storeAligns (uint iDir, uint Shift, uint Nrep, uint L, uint indStartEnd[2], uint iFrag) {//fill in alignment data
  ...
  #define OPTIM_STOREaligns_SIMPLE
    ...
    int iP;
    for (iP=nP-1; iP>=0; iP--) {
        if ( PC[iP].rStart<=rStart ) {
            if ( (PC[iP].rStart==rStart) && PC[iP].Length<L ) continue;
            if ( (PC[iP].rStart==rStart) && PC[iP].Length==L ) return; //same alignment as before, do not store!
            break;
        };
    };
...
//         if ( PC[iP3][0]<=rStart ) {
//             if ( (PC[iP3].rStart==rStart) && PC[iP3].Length<L ) continue;
//             if ( (PC[iP3].rStart==rStart) && PC[iP3].Length==L ) iP3=-1; //same alignment as before, do not store!
//             break;
//         };
//     };
    ...
    for (iP1=0; iP1<nP; iP1++) {
        if ( (PC[iP1].rStart==rStart) && PC[iP1].Length==L ) return; //exactly the same piece
        if ( rStart >= PC[iP1].rStart ) {//is new seed within an old seed
            if ( rStart+L <= PC[iP1].rStart+PC[iP1].Length ) {//new seed is within the old piece
                //decide whether to keep the new one
                if ( (PC[iP1].Nrep==Nrep)) {//seeds map the same number of times  == to the same loci
                    if (nRemove>0) {//debug
        ...
        if ( rStart <= PC[iP1].rStart )  {//is old seed within new seed
            if ( rStart+L >= PC[iP1].rStart+PC[iP1].Length ) {//old piece is within the new piece
                //decide whether to keep the new piece
                if ( (PC[iP1].Nrep==Nrep)) {//seeds map the same number of times  == to the same loci
                    PC[iP1].Dir=-1;//do not keep the old piece
                    ++nRemove;
                };
            };
        };

        if ( iP2==-1 && ( rStart < PC[iP1].rStart || (rStart == PC[iP1].rStart && L>PC[iP1].Length) ) ) {
            iP2=iP1;
        };
...
//     };
    ...
    if (nRemove==0) {//add piece
        ...
        for (int ii=0; ii<nP; ii++) {//move old entries to free space for the new one
            ...
            if ( PC[ii].Dir != -1) {//move this record to the new place
                if (ii!=iP3) {
    ...
    //store new piece
    PC[iP].rStart=rStart;  //alignment read-start
    PC[iP].Length=L;       //alignment length
    PC[iP].Dir    = iDir; //direction
    PC[iP].Nrep   = Nrep; //repeat number - for both strands
    PC[iP].SAstart= indStartEnd[0]; //SA index 1
    PC[iP].SAend  = indStartEnd[1]; //SA index 2
    PC[iP].iFrag  = iFrag;
