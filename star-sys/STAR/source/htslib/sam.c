...
#define _parse_warn(cond, msg) if ((cond) && hts_verbose >= 2) fprintf(stderr, "[W::%s] " msg "\n", __func__)
	...
	while (p < s->s + s->l) {
		...
		if (type == 'A' || type == 'a' || type == 'c' || type == 'C') {
			...
			else if (type == 'C') while (q + 1 < p) { uint8_t  x = (unsigned long)(strtol(q + 1, &q, 0)); kputc_(x, &str); }
			else if (type == 's') while (q + 1 < p) { int16_t  x = strtol(q + 1, &q, 0); kputsn_(&x, 2, &str); }
			else if (type == 'S') while (q + 1 < p) { uint16_t x = (unsigned long)(strtol(q + 1, &q, 0)); kputsn_(&x, 2, &str); }
			else if (type == 'i') while (q + 1 < p) { int32_t  x = strtol(q + 1, &q, 0); kputsn_(&x, 4, &str); }
			else if (type == 'I') while (q + 1 < p) { uint32_t x = (unsigned long)(strtol(q + 1, &q, 0)); kputsn_(&x, 4, &str); }
