...
{//generates CIGARp string for the transcript with "p" operation for PE reads
 //p is a special CIGAR operation to encode gap between mates. This gap is negative for overlapping mates.
    ...
    uint trimL=exons[0].R - (exons[0].R<readLengthOriginal[leftMate] ? 0 : readLengthOriginal[leftMate]+1);
    if (trimL>0) {
    ...
    for (uint ii=0;ii<nExons;ii++) {//cycle over all exons, record CIGAR
        if (ii>0) {//record gaps
            uint gapG=exons[ii].G-(exons[ii-1].G+exons[ii-1].L);

            if (exons[ii].G >= (exons[ii-1].G+exons[ii-1].L) ) {//
                if (canonSJ[ii-1]==-3) {//gap between mates
                    //soft clipping of the second mate
                    uint s1=readLengthOriginal[leftMate]-(exons[ii-1].R+exons[ii-1].L);
                    uint s2=exons[ii].R-(readLengthOriginal[leftMate]+1);
                    if (s1>0){
                        samStreamCIGAR << s1 << "S";
                    };
                    ...
                    uint gapR=exons[ii].R-exons[ii-1].R-exons[ii-1].L; //gapR>0 always
                    if (gapR>0){
                ...
                samStreamCIGAR << "-" << (exons[ii-1].G+exons[ii-1].L) - exons[ii].G << "p";
            };
        };
        samStreamCIGAR << exons[ii].L << "M";
    };


    trimL=(exons[nExons-1].R<readLengthOriginal[leftMate] ? readLengthOriginal[leftMate] : readLengthPairOriginal) - exons[nExons-1].R-exons[nExons-1].L;
    if ( trimL > 0 ) {
